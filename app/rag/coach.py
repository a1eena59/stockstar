...
# Initialize Groq client (this is lightweight — OK)
groq_client = Groq(api_key=GROQ_API_KEY)

# Lightweight in-memory principles store
principles_chunks = None


def load_principles():
    """
    Lightweight loader for principles.txt.
    Compatible replacement for Chroma version.
    """
    global principles_chunks

    if principles_chunks is not None:
        return len(principles_chunks)
    ...
    chunks = [chunk.strip() for chunk in content.split("\n\n") if chunk.strip()]
    principles_chunks = chunks

    print(f"✅ Lightweight RAG loaded {len(chunks)} market principles")
    return len(chunks)


def retrieve_relevant_principles(headline: str, n_results: int = 3) -> list:
    """
    Lightweight semantic-ish retrieval using keyword overlap.
    Fully compatible with previous pipeline.
    """
    global principles_chunks

    if principles_chunks is None:
        load_principles()

    words = set(re.findall(r"\w+", headline.lower()))

    scored = []
    for chunk in principles_chunks:
...
def generate_debrief(
    ...
    prediction_context = (
        "The player correctly predicted the most affected sector."
        if player_predicted_correctly
        else "The player did not correctly predict the most affected sector."
    )
...
def generate_bridge_advice(
    ...
    """
    Generate personalized post-game financial advice.
    """
    ...
    if not invests:
        ...
        situation = (
            f"The player invests through a {account_type} and estimates "
            f"{tech_allocation}% of their portfolio is in Tech stocks."
        )
